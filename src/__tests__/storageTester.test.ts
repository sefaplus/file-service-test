...
describe('Storage adapter', () => {
  ...
  test('saveFile', () => {
    ...
    return storage.saveFile(file, filename, metadata).then((data) => {
      ...
      const serverRoot = Server.rootFolder.substring(0, Server.rootFolder.length - 4);

      /* Expected uploaded file path */
      const expectedPath = `${serverRoot}/${config.storage.localSavePath}${filename}.${testConstants.extension}`;
